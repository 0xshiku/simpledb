...
func TestFileMgrWriteRead(t *testing.T) {
	...
	checkFileContent(t, filepath.Join(dataDir, testFile), "aaaaaaaaaaaaa")
	...
	checkFileContent(t, filepath.Join(dataDir, testFile), "aaaaaaaaaaaaaaabbbbbbbbbbbbbb")

	// Write to block 0
	data = "cccccccccccc"
	checkWrite(t, mgr, blockZero, data)
	checkRead(t, mgr, blockZero, data)
	checkFileContent(t, filepath.Join(dataDir, testFile), "ccccccccccccbbbbbbbbbbbbbb")
}
